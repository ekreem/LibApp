    ...
    class Library
    {
        ...
        public void UpdateBook()
        {
            ...
            for (int i = 0; i < Books.Length - 1; i++)
            {
                if (Books[i].Id == id)
                {
                    Books[i].Name = createbook.Name;
                    Books[i].AuthorName = createbook.AuthorName;
                    Books[i].Price = createbook.Price;
                }
        ...
        public void GetAllBooks() {
            for (int i = 0; i < Books.Length; i++)
            {
                Console.WriteLine($" {Books[i].AuthorName} - {Books[i].Name} - {Books[i].Price} "); 
            }
        }
    }
}
